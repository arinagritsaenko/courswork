...
namespace TCPConnectionAPI_C_sharp_
{
    class DatabaseContext : IMainDBPermission
    {
        ...
        virtual public int CreateCarrier(Carrier obj)
        {
            DBconnection.Configuration.ProxyCreationEnabled = true;
            DBconnection.CarrierSet.Add(obj);
            DBconnection.SaveChanges();
            DBconnection.Configuration.ProxyCreationEnabled = false;
            return obj.Id;
        }
        ...
        virtual public bool DeleteCarriersWhere(Func<Carrier, bool> comparer)
        {
            var planeSet = FindCarriersWhere(comparer);
            if (planeSet.Count == 0) { return false; }
            else
            {
                DBconnection.Configuration.ProxyCreationEnabled = true;
                DBconnection.CarrierSet.RemoveRange(planeSet);
                DBconnection.Configuration.ProxyCreationEnabled = false;
                DBconnection.SaveChanges();
                return true;
            }
        ...
        virtual public List<Carrier> FindCarriersWhere(Func<Carrier, bool> comparer)
        {
            return DBconnection.CarrierSet
                .Include("RateSet")
                .Where(comparer)
                .ToList();
        }

        virtual public bool UpdateCarrier(Carrier newVersion)
        {
            DBconnection.Configuration.ProxyCreationEnabled = true;
            var buf = DBconnection.CarrierSet.Find(newVersion.Id);
            ...
            else
            {
                ...
                buf.Traffic = newVersion.Traffic;
                buf.AmountOfShips = newVersion.AmountOfShips;
                DBconnection.SaveChanges();
                DBconnection.Configuration.ProxyCreationEnabled = false;
                return true;
            }
