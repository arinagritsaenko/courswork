        ...
        protected virtual void ClientHandler(ConnectedUserInfo user)
        {
            ...
            using (IClientAbilityProtocol clientProtocol = new ClientAbilityProtocol())
            {
                while (true)
                {
                    switch (_protocol.ReceiveCommand(user.ConnectionSocket))
                    {
                        case CommandsToServer.FindCarrierByRate:
                            {
                                var param = float.Parse(_protocol.ReceiveString(user.ConnectionSocket));
                                _protocol.SendCollection(clientProtocol.FindCarriersWhere(c => c.TotalRate == param), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.FindCarrierByName:
                            {
                                var param = _protocol.ReceiveString(user.ConnectionSocket);
                                _protocol.SendCollection(clientProtocol.FindCarriersWhere(c => c.Name == param), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.FindCarrierByRegistrationNumber:
                            {
                                var param = int.Parse(_protocol.ReceiveString(user.ConnectionSocket));
                                _protocol.SendCollection(clientProtocol.FindCarriersWhere(c => c.RegistrationNumber == param), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.FindCarrierByAmountOfShips:
                            {
                                var param = int.Parse(_protocol.ReceiveString(user.ConnectionSocket));
                                _protocol.SendCollection(clientProtocol.FindCarriersWhere(c => c.AmountOfShips == param), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.FindCarrierByTraffic:
                            {
                                var param = int.Parse(_protocol.ReceiveString(user.ConnectionSocket));
                                _protocol.SendCollection(clientProtocol.FindCarriersWhere(c => c.Traffic == param), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.GetAllCarriers:
                            {
                                _protocol.SendCollection(clientProtocol.FindCarriersWhere(c => c != null), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.PreviousRoom:
                            {
        ...
        protected virtual void AdminHandler(ConnectedUserInfo user)
        {
            ...
            using (IAdminAbilityProtocol adminProtocol = new AdminAbilityProtocol())
            {
                while (true)
                {
                    switch (_protocol.ReceiveCommand(user.ConnectionSocket))
                    {
                        case CommandsToServer.CreateCarrier:
                            {
                                if (adminProtocol.CreateCarrier(_protocol.ReceiveObject<Carrier>(user.ConnectionSocket)))
                                    _protocol.SendAnswerFromServer(AnswerFromServer.Successfully, user.ConnectionSocket);
                                else
                                    _protocol.SendAnswerFromServer(AnswerFromServer.Error, user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.ModifyCarrier:
                            {
                                if (adminProtocol.ModifyCarrier(_protocol.ReceiveObject<Carrier>(user.ConnectionSocket)))
                                    _protocol.SendAnswerFromServer(AnswerFromServer.Successfully, user.ConnectionSocket);
                                else
                                    _protocol.SendAnswerFromServer(AnswerFromServer.Error, user.ConnectionSocket);
                                break;
                        ...
                        case CommandsToServer.DeleteCarrier:
                            {
                                int id = int.Parse(_protocol.ReceiveString(user.ConnectionSocket));
                                if (adminProtocol.DeleteCarriersWhere(c => c.Id == id))
                                    _protocol.SendAnswerFromServer(AnswerFromServer.Successfully, user.ConnectionSocket);
                                else
                                    _protocol.SendAnswerFromServer(AnswerFromServer.Error, user.ConnectionSocket);
                                break;
                        ...
                        case CommandsToServer.FindCarrierByRate:
                            {
                                var param = float.Parse(_protocol.ReceiveString(user.ConnectionSocket));
                                _protocol.SendCollection(adminProtocol.FindCarriersWhere(c => c.TotalRate == param), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.FindCarrierByName:
                            {
                                var param = _protocol.ReceiveString(user.ConnectionSocket);
                                _protocol.SendCollection(adminProtocol.FindCarriersWhere(c => c.Name == param), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.FindCarrierByRegistrationNumber:
                            {
                                var param = int.Parse(_protocol.ReceiveString(user.ConnectionSocket));
                                _protocol.SendCollection(adminProtocol.FindCarriersWhere(c => c.RegistrationNumber == param), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.FindCarrierByAmountOfShips:
                            {
                                var param = int.Parse(_protocol.ReceiveString(user.ConnectionSocket));
                                _protocol.SendCollection(adminProtocol.FindCarriersWhere(c => c.AmountOfShips == param), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.FindCarrierByTraffic:
                            {
                                var param = int.Parse(_protocol.ReceiveString(user.ConnectionSocket));
                                _protocol.SendCollection(adminProtocol.FindCarriersWhere(c => c.Traffic == param), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.GetAllCarriers:
                            {
                                _protocol.SendCollection(adminProtocol.FindCarriersWhere(c => c != null), user.ConnectionSocket);
                                break;
                            }
                        ...
                        case CommandsToServer.CreateReportAboutCarrier:
                            {
                                _protocol.SendString(adminProtocol.CreateReportAboutCarrier(), user.ConnectionSocket);
                                break;
                            }
                ...
                while (true)
                {
                    switch (_protocol.ReceiveCommand(user.ConnectionSocket))
                    {
                        case CommandsToServer.FindCarrierByRate:
                            {
                                var param = float.Parse(_protocol.ReceiveString(user.ConnectionSocket));
                                _protocol.SendCollection(expertProtocol.FindCarriersWhere(c => c.TotalRate == param), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.FindCarrierByName:
                            {
                                var param = _protocol.ReceiveString(user.ConnectionSocket);
                                _protocol.SendCollection(expertProtocol.FindCarriersWhere(c => c.Name == param), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.FindCarrierByRegistrationNumber:
                            {
                                var param = int.Parse(_protocol.ReceiveString(user.ConnectionSocket));
                                _protocol.SendCollection(expertProtocol.FindCarriersWhere(c => c.RegistrationNumber == param), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.FindCarrierByAmountOfShips:
                            {
                                var param = int.Parse(_protocol.ReceiveString(user.ConnectionSocket));
                                _protocol.SendCollection(expertProtocol.FindCarriersWhere(c => c.AmountOfShips == param), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.FindCarrierByTraffic:
                            {
                                var param = int.Parse(_protocol.ReceiveString(user.ConnectionSocket));
                                _protocol.SendCollection(expertProtocol.FindCarriersWhere(c => c.Traffic == param), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.GetAllCarriers:
                            {
                                _protocol.SendCollection(expertProtocol.FindCarriersWhere(c => c != null), user.ConnectionSocket);
                                break;
                            }
                        case CommandsToServer.RateCarrier:
                            {
                                ...
                                var buf = expertProtocol.FindCarriersWhere(c => c.Id == entityId)[0];
